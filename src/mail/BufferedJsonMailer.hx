 ...
 * @author fbarbut
 */
class BufferedJsonMailer implements IMailer
{
	var conf : Dynamic;
	var type : String;
	...
	public function send(m:sugoi.mail.IMail,?params:Dynamic,?callback:MailerResult->Void):Void{
		...
		var bm = new db.BufferedJsonMail();
		bm.setHeaders( m.getHeaders() );
		bm.title = m.getTitle();
		bm.htmlBody = m.getHtmlBody();
		bm.textBody = m.getTextBody();
		bm.setRecipients( m.getRecipients() );
		bm.setSender( m.getSender() );		
