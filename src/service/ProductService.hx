...
class ProductService {
	...
	public static function batchDisableProducts(productIds:Array<Int>) {
		...
		for (pid in data.pids) {
			...
			if (Lambda.find(products, function(p) return p.id == pid) == null)
				throw 'product $pid is not in this contract !';
	...
	**/
	...
	public static function getForm(?product:db.Product, ?catalog:db.Catalog):sugoi.form.Form {
		...
		f.getElement("variablePrice")
			.docLink = "https://wiki.amap44.org/fr/app/admin-gestion-produits#option-prix-variable-selon-pes%C3%A9e-uniquement-contrat-variable";
		...
		if (product.catalog.hasStockManagement()) {
			...
			stockElement.label += " (x&nbsp;" + product.qt + "&nbsp;" + Formatting.unit(product.unitType) + ")";
			...
			f.addElement(new form.StockTrackingPerDistribForm('stockTrackingPerDistrib', t._("Stock per distribution configuration"),
				product.stockTrackingPerDistrib == null ? null : product.stockTrackingPerDistrib.getIndex(), stockElement, distributionsStocks, distribs),
		...
		var data:sugoi.form.ListData.FormData<Float> = group.getVatRates().map(r -> {
			label: r.label,
			value: r.value,
			desc: null,
			docLink: null
				...
				case PerDistribution:
					switch product.stockTrackingPerDistrib {
						...
						case FrequencyBased: {
								...
								productDistribStock.startDistribution = db.Distribution.manager.get(Std.parseInt(App.current.params.get(f.name
									+ "_firstDistrib")));
	...
	public static function check(product:db.Product) {
		...
		if (product.bulk) {
			if (product.unitType == null)
				throw new Error("Vous devez définir l'unité de votre produit si l'option 'vrac' est activée");
			if (product.qt == null)
				throw new Error("Vous devez définir une quantité si l'option 'vrac' est activée");
			if (product.multiWeight)
				throw new Error("Un produit en vrac ne peut pas être aussi en multi-pesée.");
		}
			...
			if (product.stock == null) {
				...
				for (d in nextDistribs) {
					...
					if (stockValue < 0) {
						...
						throw new Error(t._("Stock can't be less than ::minStock:: for ::product:: on distribution ::ddate:: because of existing orders.",
