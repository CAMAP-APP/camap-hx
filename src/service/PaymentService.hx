...
class PaymentService {
	...
	public static function updateUserBalance(user:db.User, group:db.Group) {
		...
		**/
		var b = sys.db.Manager.cnx.request('SELECT SUM(Operation.amount) FROM Operation, Subscription WHERE Operation.userId=${user.id} and Operation.groupId=${group.id} and Operation.subscriptionId = Subscription.id and year(Subscription.endDate) > 2021').getFloatResult(0);
