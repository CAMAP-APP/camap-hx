...
class SubscriptionService {
	...
	**/
	...
	public static function getOpenDistribsForSubscription(user:db.User, catalog:db.Catalog,
			currentOrComingSubscription:db.Subscription):Array<db.Distribution> {
		...
		if (currentOrComingSubscription != null) {
			...
			var now = Date.now();
			openDistribs = db.Distribution.manager.search($catalog == catalog && $orderStartDate <= now && $orderEndDate > now, {orderBy: date}, false)
				.array();
		}
	...
	**/
	public static function getSubscriptionDistributions(subscription:db.Subscription, ?type = 'all'):Array<db.Distribution> {
		if (subscription == null)
			throw new Error('La souscription n\'existe pas');

		var subscriptionDistribs = new List();
		if (type == "all" || type == "allIncludingAbsences") {
			subscriptionDistribs = db.Distribution.manager.search($catalog == subscription.catalog && $date >= subscription.startDate
				&& $end <= subscription.endDate, {
					orderBy: date
				}, false);
		} else if (type == "open") {
			var now = Date.now();
			subscriptionDistribs = db.Distribution.manager.search($catalog == subscription.catalog && $orderStartDate <= now && $orderEndDate > now
				&& $date >= subscription.startDate && $end <= subscription.endDate,
				{
					orderBy: date
				}, false);
		} else if (type == "past") {
			...
			subscriptionDistribs = db.Distribution.manager.search($catalog == subscription.catalog && $end <= endOfToday && $date >= subscription.startDate
				&& $end <= subscription.endDate, false);
		}
	...
	public static function getSubscriptionRemainingDistribsNb(subscription:db.Subscription):Int {
		...
		remainingSubscriptionDistribs = db.Distribution.manager.search($catalog == subscription.catalog && $date >= Date.now() && $end <= subscription.endDate,
			false);
	...
	public static function getSubscriptionDistribsNb(subscription:db.Subscription, ?type:String, ?excludeAbsences = true):Int {
		...
		if (type == null) {
			subscriptionDistribsNb = db.Distribution.manager.count($catalog == subscription.catalog && $date >= subscription.startDate
				&& $end <= subscription.endDate);
		} else if (type == "past") {
			var now = Date.now();
			var endOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59);
			subscriptionDistribsNb = db.Distribution.manager.count($catalog == subscription.catalog && $date <= endOfToday
				&& $date >= subscription.startDate && $end <= subscription.endDate);
		}
	...
	**/
	...
	/**
		Get contract constraints (no subscription for the current user)
	**/
	public static function getMinOrdersTotalPoportional(catalog:db.Catalog, subscription:db.Subscription):Float {
		if (catalog.isVariableOrdersCatalog() && catalog.catalogMinOrdersTotal > 0) {
	...
	function checkSubscriptionBeforeUpdate(subscription:db.Subscription, ?previousStartDate:Date) {
		...
		if (!adminMode) {
			...
			if (newSubscriptionStartDate == null) {
				throw TypedError.typed('Toutes les distributions futures sont déjà fermées, ou il n\'existe aucune distribution dans le futur.',
					InvalidParameters);
			}
			if (subscription.id == null) {
				// new sub
				if (subscription.startDate.getTime() < newSubscriptionStartDate.getTime()) {
					throw TypedError.typed('La date de début de la souscription ne doit pas être avant la date de la prochaine distribution : ${Formatting.dDate(newSubscriptionStartDate)}',
						InvalidParameters);
				}
			} else {
				// existing sub
				if (previousStartDate.toString() != subscription.startDate.toString()) {
					if (Date.now().getTime() <= subscription.startDate.getTime()
						&& subscription.startDate.getTime() < newSubscriptionStartDate.getTime()) {
						throw TypedError.typed('La date de début de la souscription ne doit pas être avant la date de la prochaine distribution : ${Formatting.dDate(newSubscriptionStartDate)}',
							InvalidParameters);
					}
				}
		...
		var catalogStartDate = new Date(subscription.catalog.startDate.getFullYear(), subscription.catalog.startDate.getMonth(),
			subscription.catalog.startDate.getDate(), 0, 0, 0);
		var catalogEndDate = new Date(subscription.catalog.endDate.getFullYear(), subscription.catalog.endDate.getMonth(),
			subscription.catalog.endDate.getDate(), 23, 59, 59);
		if (subscription.startDate.getTime() < catalogStartDate.getTime()
			|| subscription.startDate.getTime() >= catalogEndDate.getTime()) {
		...
		if (subscription.id == null) { // We need to check there the id as $id != null doesn't work in the manager.search
			...
			subscriptions1 = db.Subscription.manager.search($user == subscription.user
			...
			subscriptions2 = db.Subscription.manager.search($user == subscription.user && $catalog == subscription.catalog
				&& $startDate <= subscription.endDate && $endDate >= subscription.endDate,
				false);
			// Looking for existing subscriptions with a time range included in the time range of the about to be created subscription
			subscriptions3 = db.Subscription.manager.search($user == subscription.user
				&& $catalog == subscription.catalog
				&& $startDate >= subscription.startDate
		...
		if (subscriptions1.length != 0 || subscriptions2.length != 0 || subscriptions3.length != 0) {
			...
			throw TypedError.typed('Il y a déjà une souscription pour ce membre pendant la période choisie.' + "(" + subs.join(',') + ")",
				OverlappingSubscription);
		}

		if (subscription.id != null
			&& hasPastDistribOrdersOutsideSubscription(subscription)
			&& subscription.catalog.isVariableOrdersCatalog()) {
			throw TypedError.typed('La nouvelle période sélectionnée exclue des commandes déjà passées, Il faut élargir la période sélectionnée $subName.',
				PastOrders);
		}

		if (!adminMode && hasPastDistribsWithoutOrders(subscription)) {
			throw TypedError.typed('La nouvelle période sélectionnée inclue des distributions déjà passées auxquelles le membre n\'a pas participé, Il faut choisir une date ultérieure $subName.',
				PastDistributionsWithoutOrders);
		}
	}
	...
	**/
	public static function getCatalogMinOrdersTotal(catalog:db.Catalog, ?subscription:db.Subscription):Float {
		...
		if (subscription != null) {
			...
			subscriptionDistribsNb = db.Distribution.manager.count($catalog == catalog
				&& $date >= SubscriptionService.getNewSubscriptionStartDate(catalog));
		}

		var catalogAllDistribsNb = db.Distribution.manager.count($catalog == catalog);
		if (catalogAllDistribsNb == 0)
			return null;
		var ratio = subscriptionDistribsNb / catalogAllDistribsNb;
	...
	public static function checkVarOrders(ordersByDistrib:Map<db.Distribution, Array<CSAOrder>>, ?subscription:db.Subscription):Bool {
		...
		if (catalog.distribMinOrdersTotal > 0) {
			...
			for (distrib in keys) {
				...
				if (distrib.orderEndDate.getTime() < now)
					continue;
	...
	**/
	...
	public static function deleteSubscription(subscription:db.Subscription) {
		...
		for (operation in getOperations(subscription, true))
			operation.delete();
	...
	public static function hasPastDistribOrdersOutsideSubscription(subscription:db.Subscription):Bool {
		...
		for (order in orders) {
			if (order.distribution != null) {
				if (order.distribution.date.getTime() <= endOfToday.getTime()
					&& (order.distribution.date.getTime() < subscription.startDate.getTime()
						|| order.distribution.date.getTime() > subscription.endDate.getTime())) {
					return true;
				}
			}
		}
	...
	public static function hasPastDistribsWithoutOrders(subscription:db.Subscription):Bool {
		if (!hasPastDistributions(subscription)) {
			...
			if (/*( subscription.catalog.type == db.Catalog.TYPE_VARORDER && subscription.catalog.requiresOrdering ) ||*/ subscription.catalog.type == db.Catalog.TYPE_CONSTORDERS) {
			...
			for (order in ordersData) {
				if (order.quantity > 0) {
					...
					if (order.userId2 != null && order.userId2 != 0) {
						...
						if (user2 == null)
							throw new Error('Impossible de trouver l\'utilisateur #${order.userId2}');
						if (subscription.user.id == user2.id)
							throw new Error("Les deux comptes sélectionnés doivent être différents");
						if (!user2.isMemberOf(product.catalog.group))
							throw new Error('L\'utilisateur #${user2} ne fait pas partie de ce groupe');
	...
	**/
      ...
      if (( subscription.catalog.distribMinOrdersTotal>0 && (defaultOrders == null || defaultOrders.length == 0)) || !adminMode ) {
		...
		if (subscription.catalog.isConstantOrdersCatalog()) {
			if (hasPastDistribOrders(subscription) && !adminMode) {
				throw TypedError.typed('Il y a des commandes pour des distributions passées. Les commandes du passé ne pouvant être modifiées, il faut recréer une nouvelle souscription avec une nouvelle commande.',
					SubscriptionServiceError.PastOrders);
			}
		}
	...
	public static function createOrUpdateTotalOperation(subscription:db.Subscription):db.Operation {
		if (subscription == null)
			throw new Error('Pas de souscription fournie.');
	...
	public static function transferBalance(fromSubscription:db.Subscription, toSubscription:db.Subscription) {
		...
		if (fromSubscription == null || toSubscription == null)
			throw new Error('Pas de souscriptions fournies.');
		if (fromSubscription.user.id != toSubscription.user.id)
			throw new Error('Le transfert est possible uniquement pour un même membre.');
