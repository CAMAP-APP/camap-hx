...
class App extends sugoi.BaseApp {
	 ...
	 */
	public static var VERSION = ([3, 0, 10] : Version) /*.withPre(GitMacros.getGitShortSHA(), GitMacros.getGitCommitDate())*/;
	...
	public function setTheme() {
		var defaultTheme:Theme = {
			...
			email: {senderEmail: 'noreply@camap.amap44.org',
				...
				+ App.config.HOST
				+
				'/img/emails/website.png" alt="Site web" height="25" style="width:auto!important; height:25px!important; vertical-align:middle" valign="middle" width="auto"/>Site web
                    </a>
				...
				+ App.config.HOST
				+
				'/img/emails/facebook.png" alt="Facebook" height="25" style="width:auto!important; height:25px!important; vertical-align:middle" valign="middle" width="auto"/>Facebook
                    </a>
                </div>'},
			terms: {
	...
	**/
	...
	public static function sendMail(m:sugoi.mail.Mail, ?group:db.Group, ?sender:{email:String, ?name:String, ?userId:Int}) {
		if (group == null)
			group = App.current.user == null ? null : App.current.user.getGroup();
		if (group != null)
			m.setSender(group.contact == null ? App.current.getTheme().email.senderEmail : group.contact.email, group.name);
		if (sender != null)
