 ...
 */
@:index(sdate,cdate)
class BufferedJsonMail extends sys.db.Object
{
	public var id : SId;
    ...
    public function setSender(sender:{name:String,email:String,?userId:Int}){
        this.sender = haxe.Json.stringify(sender);
    }
    public function setRecipients(recipients:Array<{name:String,email:String,?userId:Int}>){
        this.recipients = haxe.Json.stringify(recipients);
    }
