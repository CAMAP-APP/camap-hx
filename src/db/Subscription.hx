...
class Subscription extends Object {
	...
	**/
	...
	public function getDefaultOrdersToString():String {
		if (this.defaultOrders == null)
			return 'Aucune commande par défaut définie';
	...
	**/
	public function getPossibleAbsentDistribs():Array<db.Distribution> {
		if (this.catalog.absencesStartDate == null)
			return [];
		...
		var subDistributions = db.Distribution.manager.search($catalog == this.catalog && $date >= this.startDate && $end <= this.endDate && $quantities == 1,
			{orderBy: date}, false);
