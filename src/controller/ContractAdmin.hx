...
class ContractAdmin extends Controller {
	...
	function doDefault(?args:{old:Bool}) {
		...
		if (contractsToFix.length > 0) {
			app.session.addMessage('Attention, la gestion des "frais au pourcentage de la commande" va disparaître le 1er Février 2023.<br/>Les catalogues suivants l\'utilisent : <b>${contractsToFix.map(c -> c.name).join(", ")}</b><br/><a href="https://wiki.cagette.net/basculecommissioncatalogue" target="_blank">Cliquez ici pour connaître un alternative.</a>',
				true);
		}
	...
	function doEdit(catalog:db.Catalog) {
		...
		if (!app.user.isContractManager(catalog))
	...
	**/
	...
	function doOrdersByTimeFrame(?from:Date, ?to:Date /*, ?place:db.Place*/) {
		...
		if (from == null) {
			...
			if (distribs.length == 0)
				throw Error("/contractAdmin/ordersByTimeFrame", t._("There is no delivery at this date"));
	...
	function doOrders(catalog:db.Catalog, ?args:{d:db.Distribution, ?delete:db.UserOrder}) {
		...
		if (App.current.params.get("csv") == "1") {
			...
			var exportName = catalog.group.name
				+ " - "
				+ t._("Delivery ::contractName:: ", {contractName: catalog.name})
				+ args.d.date.toString().substr(0, 10);
			sugoi.tools.Csv.printCsvDataFromObjects(data, ["name", "productName", "price", "quantity", "fees", "total", "paid"],
				exportName + " - " + t._("Per member"));
		}
	}
	...
	function doDuplicate(catalog:db.Catalog) {
		sendNav(catalog);
		if (!app.user.canManageContract(catalog))
			throw Error("/", t._("You do not have the authorization to manage this catalog"));
		...
		var catalogTypes = [
			{label: 'Contrat AMAP classique', value: 0},
			{label: 'Contrat AMAP variable', value: 1}
		];
		...
		if (form.checkToken()) {
			...
			if (catalog.vendor.user != null) {
				db.UserGroup.getOrCreate(catalog.vendor.user, catalog.group);
			}
			...
			// copy roles
			var roles = VolunteerService.getRolesFromGroup(app.user.getGroup()).filter(function(role) return role.catalog == catalog);
			for (r in roles) {
				var role = new db.VolunteerRole();
				role.name = r.name;
				role.catalog = nc;
	...
	function doOrdersByProduct(contract:db.Catalog, args:{?d:db.Distribution}) {
		...
		if (d == null)
			throw Error("/contractAdmin/orders/" + contract.id, t._("There is no delivery in this catalog, please create at least one distribution."));
	...
	function doOrdersByProductList(contract:db.Catalog, args:{d:db.Distribution}) {
		sendNav(contract);
		if (!app.user.canManageContract(contract))
			throw Error("/", t._("Forbidden access"));
		if (args.d.catalog.id != contract.id)
			throw 'Distribution does not belong to this catalog';
		...
		switch (args.stat) {
			case 0:
				...
				if (pids.length == 0) {
					...
					view.anciennete = sys.db.Manager.cnx.request("select YEAR(u.cdate) as uyear ,count(DISTINCT u.id) as cnt from User u, UserOrder up where up.userId=u.id and up.productId IN ("
						+ pids.join(",")
						+ ") group by uyear;")
						.results();
				}
			...
			case 1:
				...
				var totalPrice = 0;
				if (pids.length != 0) {
					var repartition = sys.db.Manager.cnx.request("select sum(quantity) as quantity,productId,p.name,p.price from UserOrder up, Product p where up.productId IN ("
