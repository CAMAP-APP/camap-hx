...
class SubscriptionAdmin extends controller.Controller {
	...
	**/
	...
	public function doDelete(subscription:db.Subscription) {
		if (!app.user.canManageContract(subscription.catalog))
			throw Error('/', t._('Access forbidden'));
		...
		if (checkToken()) {
			try {
				...
				if (startDate == null || endDate == null) {
					throw Error('/contractAdmin/subscriptions/insert/' + catalog.id,
						"Vous devez sélectionner une date de début et de fin pour la souscription.");
				}
				...
				if (catalog.hasDefaultOrdersManagement()) {
					for (product in catalogProducts) {
						...
						if (qtyParam != "")
							quantity = Std.parseFloat(qtyParam);
						...
						if (userId2 != null && userId2 != 0) {
							...
							if (user.id == user2.id) {
								throw Error('/contractAdmin/subscriptions/insert/' + catalog.id,
									"Vous ne pouvez pas alterner avec la personne qui a la souscription.");
							}
				...
				subscriptionService.createSubscription(user, catalog, ordersData, null, absenceDistribIds, null, startDate, endDate);
	...
	**/
	...
	public function doEdit(subscription:db.Subscription) {
		...
		if (checkToken()) {
			try {
				...
				if (startDate == null || endDate == null) {
					throw Error('/contractAdmin/subscriptions/edit/' + subscription.id,
						"Vous devez sélectionner une date de début et de fin pour la souscription.");
				}
				...
				for (id in absenceDistribIds) {
					...
					if ((d != null && d.date.getTime() > endDate.getTime()) || (d != null && d.date.getTime() < startDate.getTime())) {
						throw Error('/contractAdmin/subscriptions/edit/' + subscription.id,
							"Une date d'absence est en dehors de la période de souscription. Veuillez modifier les dates d'absence avant de modifier les dates de souscription.");
					}
				}
	...
	public function doBalanceTransfer(subscription:db.Subscription) {
		...
		if (subscriptionsChoices.length == 0)
			throw Error('/contractAdmin/subscriptions/payments/' + subscription.id,
				'Ce membre n\'a pas d\'autre souscription. Veuillez en créer une nouvelle avec le même producteur.');

		if (checkToken()) {
			try {
				var subscriptionId = Std.parseInt(app.params.get('subscription'));
				if (subscriptionId == null)
					throw Error('/contractAdmin/subscriptions/balanceTransfer/' + subscription.id, "Vous devez sélectionner une souscription.");
	...
	public function doInsertPayment(subscription:db.Subscription) {
		if (!app.user.isContractManager())
			throw Error("/", t._("Action forbidden"));
		...
		form.addElement(new sugoi.form.elements.Html("subscription",
			'<div class="control-label" style="text-align:left;"> ${subscription.catalog.name} - ${subscription.catalog.vendor.name} </div>', 'Souscription'));

		form.addElement(new sugoi.form.elements.StringInput("name", "Libellé", "Paiement", false));
		var amount:Float = null;
		if (subscription.getBalance() < 0)
			amount = Math.abs(subscription.getBalance());
		...
		for (paymentType in paymentTypes) {
			...
			if (paymentType.type == Check.TYPE)
				selected = Check.TYPE;
		}
