...
class User extends Controller {
	...
	@logged
	@tpl("user/choose.mtt")
	function doChoose(?args:{group:db.Group, ?redirect:String}) {
		// home page
		app.breadcrumb = [];
		...
		if (args != null && args.group != null) {
			...
			throw Redirect(args.redirect != null ? args.redirect : '/home');
		}
	...
	function doForgottenPassword(?key:String, ?u:db.User, ?definePassword:Bool) {
		...
		if (chpassform.isValid()) {
			...
			if (chpassform.getValueOf("pass1") == chpassform.getValueOf("pass2")) {
				...
				if (user.email2 != null)
					m.setRecipient(user.email2, user.getName());
	...
	**/
	function doJoingroup() {
		...
		if (group.contact != null && !app.user.isMemberOf(group)) {
			...
			var text = t._("A new member joined the group without ordering : <br/><strong>::newMember::</strong><br/> <a href='::url::'>See contact details</a>",
				{
				newMember: app.user.getCoupleName(),
				url: url
			});
			App.quickMail(group.contact.email, t._("New member") + " : " + app.user.getCoupleName(), text, group);
		}
	...
	**/
	...
	function doQuitGroup(group:db.Group, user:db.User, key:String) {
			...
			for (catalog in catalogs) {
				...
				for (d in distribs) {
					...
					if (userOrders.length > 0) {
						throw Error("/",
							"Vous ne pouvez pas quitter ce groupe car vous avez des commandes en cours.\nVeuillez contacter un responsable du groupe pour plus d'information.");
					}
				}
			}
		} else {
		...
		if (userGroup != null && userGroup.balance < 0)
			throw Error("/",
				"Vous ne pouvez pas quitter ce groupe car votre solde est négatif.\nVeuillez contacter un responsable du groupe pour plus d'information.");
