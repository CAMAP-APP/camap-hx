...
class Cron extends Controller {
	...
	**/
	...
	public function doHour() {
		...
		task.setTask(function() {
			...
			var multidistribs:Array<db.MultiDistrib> = db.MultiDistrib.manager.unsafeObjects('SELECT distrib.* 
			...
			for (multidistrib in multidistribs) {
				if (multidistrib.getGroup().isDisabled())
					continue;
				if (!multidistrib.group.flags.has(db.Group.GroupFlags.AllowInformationNotifs))
					continue;
				...
				if (volunteers.length != 0) {
					...
					var ddate = t._("::date:: from ::startHour:: to ::endHour::", {
						date: view.dDate(multidistrib.distribStartDate),
						startHour: view.hHour(multidistrib.distribStartDate),
						endHour: view.hHour(multidistrib.distribEndDate)
					});
		...
		task.setTask(function() {
			...
			var fromNow = now.setHourMinute(now.getHours(), 0);
			var toNow = now.setHourMinute(now.getHours() + 1, 0);
			var multidistribs:Array<db.MultiDistrib> = Lambda.array(db.MultiDistrib.manager.unsafeObjects('SELECT distrib.* 
				FROM MultiDistrib distrib INNER JOIN `Group` g
				ON distrib.groupId = g.id
			...
			for (multidistrib in vacantVolunteerRolesMultidistribs) {
				if (multidistrib.getGroup().isDisabled())
					continue;
				if (!multidistrib.group.flags.has(db.Group.GroupFlags.AllowAlertNotifs))
					continue;
				...
				var ddate = view.dDate(multidistrib.distribStartDate) + " de " + view.hHour(multidistrib.distribStartDate) + " à "
					+ view.hHour(multidistrib.distribEndDate);
		...
		task.setTask(function() {
			...
			var distributionsToCheckForMissingOrders = db.Distribution.manager.unsafeObjects('SELECT Distribution.* 
			...
			for (distrib in distributionsToCheckForMissingOrders) {
				var distribSubscriptions = db.Subscription.manager.search($catalog == distrib.catalog && $startDate <= distrib.date && $endDate >= distrib.date,
					false);
		...
		task.setTask(function() {
			...
			for (f in files) {
				// product file
				if (db.Product.manager.select($image == f) != null)
					continue;

				// entity file
				if (sugoi.db.EntityFile.manager.select($file == f) != null)
					continue;

				// TODO : remove entityFiles related to unexisting entities

				// vendor logo
				if (db.Group.manager.select($image == f) != null)
					continue;

				// group logo
				if (db.Vendor.manager.select($image == f) != null)
					continue;
	...
	**/
	...
	function distribNotif(task:TransactionWrappedTask, now:Date, hour:Int, flag:db.User.UserFlags) {
		...
		var orders = [];
		for (d in distribs) {
			if (d == null || d.catalog == null)
				continue;
			// get orders for both type of catalogs
			for (x in d.catalog.getOrders(d))
				orders.push(x);
		}
			...
			if (u.user.flags.has(flag)) {
				if (u.user.email != null) {
					...
					text = t._("Do not forget the delivery on <b>::day::</b> from ::from:: to ::to::<br/>",
						{day: view.dDate(d.distribStartDate), from: view.hHour(d.distribStartDate), to: view.hHour(d.distribEndDate)});
				...
				if (user.flags.has(db.User.UserFlags.HasEmailNotifOuverture)) {
					if (user.email != null) {
						...
						for (d in g.distributions) {
							...
							text += "<li>" + d.catalog.vendor.name + " (" + peopleName + ")<br>Catalogue: " + d.catalog.name
								+ " <br>Commandes ouvertes jusqu'au: " + view.hDate(d.orderEndDate) + " </li>";
						}
						text += "</ul>";
						task.log(text);
	...
	function sendOrdersLists(task:TransactionWrappedTask) {
		...
		var distribs = db.Distribution.manager.unsafeObjects('SELECT Distribution.* 
		FROM Distribution INNER JOIN Catalog
		ON Distribution.catalogId = Catalog.id
		AND Distribution.orderEndDate >= \'${range.from}\'
		AND Distribution.orderEndDate < \'${range.to}\';', false);
		...
		for (distri in distribs) {
			...
			var distribOrders = {
				distrib: distri,
				catalog: distri.catalog,
				vendor: distri.catalog.vendor,
				orders: [],
				newSubs: []
			}
			...
			if (lastDistrib != null)
				distribOrders.newSubs = SubscriptionService.getNewCatalogSubscriptions(contrat, lastDistrib.date).map(sub -> {
			...
			if (dest != null) {
				var m = new Mail();
				m.setSender(App.current.getTheme().email.senderEmail, App.current.getTheme().name);
				if (amap.contact != null)
					m.setReplyTo(amap.contact.email, amap.name);
