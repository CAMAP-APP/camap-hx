	...
	**/
	public function doDefault(catalog:db.Catalog) {
		...
		var out = {
			...
			documents: catalog.getVisibleDocuments(app.user).array().map(ef -> {
				name: ef.file.name,
				url: "/file/" + sugoi.db.File.makeSign(ef.file.id)
			}),
			...
			catalogMinOrdersTotal: catalog.catalogMinOrdersTotal,
		}
