	...
	override public function render() {
		...
		for( catalogId in ordersByCatalogId.keys() ) {
			...
			for ( order in ordersByCatalogId[catalogId] ) {
				...
				ordersByCatalog.push( jsx( '<Order key=${key} order=${order} currency=${props.currency} catalogType=${props.catalogType} />' ));
        ...
        var validateButton = jsx('
            <Button 
                ...
                ${CamapTheme.getIcon("chevron-right")}&nbsp;Valider
            </Button>');				
				...
				return jsx('<div key="orderBox" onKeyPress=${onKeyPress}>
							...
							<div key="footer" style=${{marginTop: 20}}>
                                ...
                                <Button
                                    ...
                                    variant={Outlined}
                                    disabled=${this.state.processing}>
									${CamapTheme.getIcon("plus")}&nbsp;&nbsp;Ajouter un produit
								</Button>			
							</div>
						</div>');
			} else {
